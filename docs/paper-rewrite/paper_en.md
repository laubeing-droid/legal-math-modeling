...
**Evidence discipline**: every count and theorem claim binds to the generated artifact docs/formal-release/theorem_inventory_v3.json (current subject `bbe0dd5`) and to named CI runs. No anchor, no claim; a missing anchor is UNKNOWN, never PASS.
...
2. **Theorem-level full coverage.** Of the 132 concepts, 57 anchor existing theorems and 75 are newly written this round — contract and Lean on both sides, 1,617 repository theorem declarations (bound to the inventory artifact), all verified by the CI authority, zero sorry.
...
**Unproved items (ordered by load)**:
1. General-completeness Lean induction for the subsumption operator (currently contract-grade);
2. Eight recorded degradations (P-042/073/083/084/097/099/127/131 at witness or closed-form level rather than general induction — upgrade paths in the ledger);
3. The delimitation theorems of the six inexpressibility classes;
4. The theorem-level rollout of the T01–T127 target spectrum (layers annotated, pending user review then item-by-item construction);
5. Real sentencing rule tables (currently the synthetic 23×23, hard-marked SYNTHETIC);
6. A real vector engine for retrieval layer one (currently contract-grade).

**Claims ledger**: 1,617 repository theorem declarations, bound to the inventory artifact (subject `bbe0dd5`), axiom audit green (named CI runs). This paper does not claim: the entirety of Chinese law formalized; approximator outputs conclusive; real data connected. Every conclusion a user sees traces through its receipt to its evidence grade; for further construction, the unproved list is itself the order of advance.
