...
2. **Theorem-level full coverage.** Of the 132 concepts, 57 anchor existing theorems and 75 are newly written this round — contract and Lean on both sides, 1,642 repository theorem declarations (bound to the inventory artifact), all verified by the CI authority, zero sorry.
...
1. T01–T127 theorem-level rollout: the machine ledger (t_p_coverage.jsonl, 127 rows) mechanically reports COVERED 0 / OPEN 127 — five rows carry recorded PARTIAL assets (T112/T122/T123/T125/T126), and PARTIAL never impersonates closure; this is the next campaign's main battleground;
...
3. Real sentencing rule tables (OPEN_EXTERNAL_DATA): only the user can supply the real table; the structure is ready and the synthetic table never impersonates it;
