...
Given a DP mechanism M(D) = f(D) + Lap(Δ/ε), consider a deterministic
"ratio-preserving boundary" transformation:
...
This is post-processing and preserves ε-DP by the post-processing theorem.
...
where y = f(D) + Lap(Δ/ε), this creates a **privacy risk**.
...
datasets. This means the set of possible outputs differs:
...
0.3·min(f(D), f(D')) and 0.3·max(f(D), f(D')) reveals which dataset
...
The ratio is **infinite**, violating the e^ε bound for any finite ε.

## Verdict: NOT Pure Post-Processing

```
┌─────────────────────────────────────────────────────────────────┐
│  DETERMINATION: Ratio-preserving floor that depends on f(D)     │
│                 is NOT valid post-processing.                    │
│                                                                  │
│  It breaks ε-DP because the floor itself is a function of        │
│  the sensitive data, not just the mechanism output.              │
└─────────────────────────────────────────────────────────────────┘
```
...
```
M_floor(D) = 0.3 · f(D) + Lap(0.3·Δ/ε_floor)
M_val(D)   = f(D) + Lap(Δ/ε_val)
Output     = max(M_floor(D), M_val(D))
```

With composition: total ε = ε_floor + ε_val.

**Result**: ε-DP is preserved but the guarantee requires accounting for
...
```
Output = max(F, f(D) + Lap(Δ/ε))
```

This is valid post-processing (F is public, independent of D).

**Result**: ε-DP preserved, but may not provide the desired ratio guarantee.

### Option C: Smooth Private Floor (APPROXIMATE DP)

Use a smooth (Lipschitz) approximation of the max operation that doesn't
...
| Approach | Privacy | Utility of Ratio Guarantee |
|----------|---------|---------------------------|
...
| Private floor + composition | (ε₁+ε₂)-DP | Moderate |
| Smooth approximation | Approximate DP | Moderate |

## Recommendation

**The current form `max(0.3·x₀, x₀ + Lap(...))` where x₀ = f(D) must be
downgraded**. Either:

1. Account for the privacy cost of computing the floor, OR
2. Use a fixed public floor, OR
3. Accept that the ratio guarantee is not achievable under pure ε-DP
